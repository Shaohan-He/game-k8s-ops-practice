...
建议围绕“为什么这样设计、怎么发现故障、怎么安全恢复”讲解，而不是只罗列组件。

1. **服务拆分**：网关统一入口，登录、匹配、房间按职责拆分；拆分带来独立发布能力，也增加网络和依赖故障面。
2. **状态分层**：Redis 保存高频临时状态，MySQL 保存需要查询和审计的记录；说明过期、数据一致性和故障恢复的取舍。
3. **异步事件**：Kafka 解耦行为日志与在线请求，但本练习仍同步等待发送结果，便于暴露失败；真实系统可采用 outbox、重试和死信队列。
4. **容器化**：固定基础镜像版本、非 root 用户、只复制必要文件、配置通过环境变量注入。
5. **Kubernetes 发布**：Deployment 滚动更新、readiness/liveness Probe、资源 request/limit、revisionHistory 和 rollout undo。
6. **可观测性**：从存活、请求率、错误率、延迟和业务事件建立基本指标；日志使用 JSON 和 Request ID。
7. **排障方法**：从入口到 Service/Endpoints/Pod，再到 Redis、MySQL、Kafka，结合指标、日志、事件验证假设。
8. **回滚边界**：应用可以回滚，破坏性数据库变更不一定能直接回滚；发布前要考虑向前、向后兼容。
9. **安全边界**：仓库中的默认密码只用于练习；真实环境需要密钥管理、网络策略、TLS、镜像扫描和最小权限。
10. **项目局限**：单节点中间件、无认证授权、无完整链路追踪、无自动扩缩容、无生产备份恢复流程。这些是可继续扩展的方向，不应描述为已经实现。

## 可继续练习的方向

- 增加 Loki/Promtail 或 EFK 日志采集。
- 接入 OpenTelemetry 和 Jaeger/Tempo。
- 增加 HPA、PodDisruptionBudget、NetworkPolicy。
- 使用 Helm 或 Kustomize overlays 管理 dev/staging 环境。
- 为 MySQL 增加备份与恢复演练。
- 增加 GitHub Actions：测试、镜像构建、Trivy 扫描和部署。
- 使用故障注入验证告警、重试和回滚流程。
